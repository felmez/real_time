import { calculations } from './canvas.mjs';
...
class Player {
  ...
  movePlayer(dir, speed) {
    if (dir === 'up') this.y - speed >= calculations.playFieldMinY ? this.y -= speed : this.y -= 0;
    if (dir === 'down') this.y + speed <= calculations.playFieldMaxY ? this.y += speed : this.y += 0;
    if (dir === 'left') this.x - speed >= calculations.playFieldMinX ? this.x -= speed : this.x -= 0;
    if (dir === 'right') this.x + speed <= calculations.playFieldMaxX ? this.x += speed : this.x += 0;
  }
  collision(item) {
    if (
